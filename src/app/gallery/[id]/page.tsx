...
import { Heart, Download, Share2, Camera, ShieldAlert, Loader2 } from 'lucide-react';
  ...
  const handleDownload = (url: string) => {
    if (gallery?.isLocked) {
      toast({
        ...
        description: "Please contact the photographer.",
      });
      return;
    }
    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="h-[60vh] relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          ...
          <h1 className="text-4xl md:text-7xl font-headline font-bold mb-4 tracking-[0.2em] uppercase">{gallery.title}</h1>
        ...
        <div className="max-w-4xl mx-auto mt-[-40px] relative z-10 px-6">
          <div className="bg-destructive/10 border border-destructive/30 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6 backdrop-blur-md">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-muted-foreground">Watermarks are active on previews. Contact the studio for original files.</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-6 pt-20">
        {gallery.items?.length === 0 ? (
          <div className="text-center py-20 bg-card/30 rounded-3xl border border-dashed border-border/50">
            <p className="text-muted-foreground italic">Your photographer is currently uploading images.</p>
          </div>
        ) : (
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">
            {gallery.items?.map((item: any) => (
              <div key={item.id} className="relative group break-inside-avoid overflow-hidden rounded-2xl border border-border/30 bg-card/50 shadow-xl" onClick={() => setSelectedImage(item.url)}>
                <div className="relative overflow-hidden cursor-zoom-in">
                  ...
                  {gallery.isLocked && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-40">
                      <div className="watermark-text text-xl">Hafash - Preview</div>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="absolute top-4 right-4 flex gap-2">
                      <Button 
                        size="icon" 
                        variant="secondary" 
                        className={`rounded-full bg-white/20 backdrop-blur-md hover:bg-primary ${item.isFavorite ? 'bg-primary text-primary-foreground' : ''}`}
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={() => setSelectedImage(null)}>
          ...
          {gallery.isLocked && (
            <div className="absolute bottom-12 left-0 right-0 text-center">
               <span className="bg-primary/20 text-primary border border-primary/50 px-6 py-2 rounded-full font-headline italic">Hafash - Original Available After Unlock</span>
            </div>
          )}
        </div>
      )}
